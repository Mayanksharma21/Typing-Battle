...
export class GameClass {
  ...
  constructor(roomId: string, io: Server, host: string) {
    this.gameId = roomId;
    this.players = [];
    this.io = io;
    this.gameHost = host;
  ...
  setupListeners(socket: Socket) {
    socket.on("start-game", async () => {
      ...
      if (this.gameHost !== socket.id) {
        return socket.emit("error", "Only Game host can start the game");
      }
      ...
      const fetchedParagraph = await generateParagraph();
      this.paragraph = fetchedParagraph;
  ...
  joinPlayer(id: string, name: string, socket: Socket) {
    if (this.gameStatus === "in-progress") {
      return socket.emit(
        "error",
        "Game has already started, Pleasw wait for it to end before joining!!"
      );
    }
